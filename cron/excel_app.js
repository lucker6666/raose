 ...
 var launch = require('./launches.json')
 var tmp = install['stats'][0];

 var datas = (function() {
   var rs = [];
   tmp['dates'].forEach(function(one, index) {
     rs.push([one.replace(/-/g, ''), tmp['data'][index], 0, 0, 0, active['stats'][0]['data'][index], 0, 0, 0, launch['stats'][0]['data'][index], 0, 0, 0, ])
   });
   return rs;
 })();
 console.log(datas)
 ...
 var fs = require('fs');
 //console.log(raw);
 fs.writeFileSync('allapp_parse.json', JSON.stringify(raw, null, 4));